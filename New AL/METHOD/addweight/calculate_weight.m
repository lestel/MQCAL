...
elseif method==1
   ...
   weight=weight_tmp;
    ...
    stand=ones(1,sample_num);
    stand(1,1:choose_num)=zeros(1,choose_num);
    ps.ymin = 0;
    for i=1:strategy_num
        if length(unique(score(i,:)))~=1
           [y,ps] = mapminmax(score(i,:),ps);
           weight_tmp(i,1)=exp(-1*sum(abs(stand-y)));
        else
